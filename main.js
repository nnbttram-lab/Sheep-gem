...
(function () {
  ...
  // Load content ---------------------------------------------------------------------------
  // On real hosting (Netlify, or any http/https server) this fetches the live JSON file,
  // so edits made through the CMS at /admin show up on reload with no rebuild step.
  // When the page is opened directly from disk (file://), browsers block fetch() of local
  // files, so we fall back to the bundled content/site-content.js snapshot instead.
  fetch("content/site-content.json")
    ...
    .then(renderSite)
    .catch((err) => {
      console.warn("Falling back to bundled site-content.js (fetch failed):", err.message);
      if (window.SITE_CONTENT) renderSite(window.SITE_CONTENT);
      else console.error("No content available — neither fetch nor window.SITE_CONTENT worked.");
    });
  ...
  // Sent immediately (not dependent on the fetch above) so the local offline editor's
  // live preview still works even when the fetch fallback path is used.
